...
public:
    ...
    Trajectory3D generateLimbTrajectory(RobotLimb* limb,
        ...
        // TODO: Ex. 5 Implement groundHeight
        double R_g = 10, center_offset = 0.5;
        double y_offset = -R_g + center_offset + sqrt(R_g*R_g - startingEEPos[0]*startingEEPos[0] - startingEEPos[2]*startingEEPos[2]);
        groundHeight = max(y_offset, 0.0);
...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        // TODO: Ex. 5 Implement groundHeight
        double R_g = 10, center_offset = 0.5;
        double y_offset = max(-R_g + center_offset + sqrt(R_g*R_g - pos.x*pos.x - pos.z*pos.z), 0.0);
        targetbFrameHeight += y_offset;
        ...
        while (t < tEnd) {
            ...
            /* + TODO: fix this */
            V3D forwardDirection = heading * robot->forward;
            V3D v1 = vForward * forwardDirection;
            V3D v2 = vSideways * (RBGlobals::worldUp.cross(forwardDirection));
            pos += getP3D(v1+v2) * dt;

            // pos.x += dt * (vForward * sin(forwardDirection[1]) + vSideways * cos(forwardDirection[1]));
            // pos.z += dt * (vForward * cos(forwardDirection[1]) - vSideways * sin(forwardDirection[1]));
            headingAngle += dt * turningSpeed;
